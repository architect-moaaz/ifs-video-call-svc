...
const views = {
    ...
    authfailure: path.join(__dirname, '../../', 'public/views/50X.html'),
};
...
async function startServer() {
    ...
    app.get(['/'], async (req, res) => {
        log.debug('hahaha', req);
        let check = await authenticate(req.query.token);
        if (check == 200) {
    ...
    async function authenticate(token) {
        var status = 401;
        var url = `https://kc-dev.intelliflow.in/realms/master/protocol/openid-connect/userinfo`;
        const options = {
            method: 'GET',
            url: url,
            headers: {
        ...
        let response1 = await axios(options)
            .then(function (response) {
                log.debug('line 310', response.status);
    ...
    app.get('/join/', async (req, res) => {
        ...
        if (check == 200) {
            if (hostCfg.authenticated && Object.keys(req.query).length > 0) {
                log.debug('Direct Joins', req.query);
    ...
    app.get('/join/:roomId', async (req, res) => {
        let check = await authenticate(req.query.token);
        log.debug('Join 323', check);

        if (check == 200) {
            log.debug('Join 302', req.query.token);
    ...
    app.post(['/api/v1/join'], (req, res) => {
        // check if user was authorized for the api call
        console.log('asjdna', req);
        log.debug('join url', req);
        let host = req.headers.host;
        let authorization = req.headers.authorization;
        let api = new ServerApi(host, authorization);
        if (!api.isAuthorized()) {
